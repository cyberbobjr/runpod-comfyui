    ...
    <div v-else>
      ...
      <div class="accordion" id="mainAccordion">
        ...
        <div class="accordion-item">
          ...
          <div id="collapseConfig" class="accordion-collapse collapse show" aria-labelledby="headingConfig" data-bs-parent="#mainAccordion">
            <div class="accordion-body">
              <div class="input-group mb-3">
                <span class="input-group-text"><i class="fas fa-folder me-1"></i></span>
                <input
                  type="text"
                  class="form-control"
                  v-model="baseDir"
              ...
              <small class="text-muted d-block mt-2">
                <i class="fas fa-info-circle me-1"></i> This is the base directory where models will be stored. Use this path in model entries with ${BASE_DIR} variable.
              </small>
            </div>
          </div>
        </div>
        ...
        <div class="accordion-item">
          ...
          <div id="collapseGroups" class="accordion-collapse collapse" aria-labelledby="headingGroups" data-bs-parent="#mainAccordion">
            <div class="accordion-body">
              <div class="d-flex justify-content-end mb-3">
                <button 
              ...
              <div class="list-group list-group-flush">
                ...
                >
                  <div>
                    <i class="fas fa-folder me-2"></i><strong>{{ groupName }}</strong>
        ...
        <div class="accordion-item">
          ...
          <div id="collapseList" class="accordion-collapse collapse" aria-labelledby="headingList" data-bs-parent="#mainAccordion">
                        ...
                        <table class="table table-bordered mb-0">
                          ...
                          <tbody>
                            <tr v-for="(model, index) in models" :key="index">
                              <td>
                                <span 
                                  :class="{ 'text-danger fw-bold': isNSFW(model) }"
                                >
                                  {{ model.dest ? model.dest.split('/').pop() : (model.git ? model.git.split('/').pop() : 'Unnamed') }}
                                </span>
                              </td>
                              <td>{{ model.type || '-' }}</td>
                              <td>
                                <span 
                                  v-for="tag in Array.isArray(model.tags) ? model.tags : (model.tags ? [model.tags] : [])" 
                                  :key="tag"
                              ...
                              <td>{{ model.size ? (model.size / 1024 / 1024).toFixed(2) + ' MB' : '-' }}</td>
                              <td>
                                <button 
                                  class="btn btn-sm btn-outline-primary me-2"
      ...
      <div class="modal fade" id="addGroupModal" tabindex="-1" :class="{ 'show d-block': showAddGroupModal }" :style="{ display: showAddGroupModal ? 'block' : 'none' }">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title"><i class="fas fa-folder-plus me-2"></i>Add New Group</h5>
              <button type="button" class="btn-close" @click="showAddGroupModal = false"></button>
            </div>
            <div class="modal-body">
              <div class="mb-3">
                <label class="form-label">Group Name</label>
                <input type="text" class="form-control" v-model="newGroupName" placeholder="Enter group name">
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" @click="showAddGroupModal = false">
      ...
      <div class="modal fade" id="editGroupModal" tabindex="-1" :class="{ 'show d-block': showEditGroupModal }" :style="{ display: showEditGroupModal ? 'block' : 'none' }">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title"><i class="fas fa-edit me-2"></i>Rename Group</h5>
              <button type="button" class="btn-close" @click="showEditGroupModal = false"></button>
            </div>
            <div class="modal-body">
              <div class="mb-3">
                <label class="form-label">New Group Name</label>
                <input type="text" class="form-control" v-model="newGroupName" placeholder="Enter new group name">
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" @click="showEditGroupModal = false">
                <i class="fas fa-times me-1"></i> Cancel
              </button>
              <button type="button" class="btn btn-primary" @click="updateGroupName">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              <i class="fas" :class="formData.dest || formData.git ? 'fa-edit' : 'fa-plus'"></i>
              {{ formData.dest || formData.git ? ' Edit Model' : ' Add New Model' }}
              <span v-if="currentGroup" class="badge bg-info ms-2">{{ currentGroup }}</span>
            </h5>
            <button type="button" class="btn-close" @click="closeModelModal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <ModelForm 
              ...
              @submit="formData.dest || formData.git ? updateModelEntry() : addModelEntry()"
