<template>
  <CommonCard>
    <template #header>
      <h3>
        <FontAwesomeIcon icon="cog" class="mr-2"/>Settings
      </h3>
    </template>
...
const passwordsMatch = computed(
  (): boolean => passwordForm.value.newPassword === confirmPassword.value
);
...
const loadConfig = async (): Promise<void> => {
  try {
    ...
    // Load API tokens
    const tokensResponse = await api.get("/auth/tokens");
...
const updateBaseDir = async (): Promise<void> => {
  ...
  try {
    ...
    showMessage("Failed to update base directory. Please try again.", "error");
  } finally {
