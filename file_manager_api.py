...
def find_models_json():
    # Premier essai: regarder dans le répertoire de base
    models_path = os.path.join(BASE_DIR, "models.json")
    if DEBUG:
        print(f"[DEBUG] Trying models.json at: {models_path}")
    
    if os.path.isfile(models_path):
        return models_path
    
    # Ensuite chercher dans le répertoire parent
    parent_dir = os.path.dirname(BASE_DIR)
    models_path = os.path.join(parent_dir, "models.json")
    if DEBUG:
        print(f"[DEBUG] Trying models.json at parent: {models_path}")
    
    if os.path.isfile(models_path):
        return models_path
    
    # Enfin, chercher dans le répertoire courant de l'application
    app_dir = os.path.dirname(os.path.abspath(__file__))
    models_path = os.path.join(app_dir, "models.json")
    if DEBUG:
        print(f"[DEBUG] Trying models.json at app dir: {models_path}")
    
    if os.path.isfile(models_path):
        return models_path
    
    # Si vous avez une copie du fichier dans votre dossier f:\runpod-comfyui
    models_path = "f:\\runpod-comfyui\\models.json"
    if DEBUG:
        print(f"[DEBUG] Trying models.json at fixed path: {models_path}")
    
    if os.path.isfile(models_path):
        return models_path
    
    # Chercher de manière récursive
    if DEBUG:
        print("[DEBUG] Searching recursively for models.json...")
    
    # Essayer de trouver models.json de manière récursive dans BASE_DIR
    for root, dirs, files in os.walk(BASE_DIR):
        if "models.json" in files:
            found_path = os.path.join(root, "models.json")
            if DEBUG:
                print(f"[DEBUG] Found models.json at: {found_path}")
            return found_path
    
    # Si après tout ça on ne trouve pas le fichier, on abandonne
    if DEBUG:
        print("[DEBUG] models.json not found anywhere")
...
def download_file(path: str, user=Depends(protected)):
    abs_path = safe_join(BASE_DIR, path)
